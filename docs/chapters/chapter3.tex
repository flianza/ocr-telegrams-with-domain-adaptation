...
Se entrenaron redes ResNet-50 \parencite{he2016deep} por cada una de las t\'ecnicas de adaptaci\'on de dominio descriptas en el cap\'itulo
\ref{Chapter2}. Adicionalmente, se entren\'o una sin ninguna de las t\'ecnicas mencionadas para ser utilizada como
base.
...
etiquetado y el de destino sin etiquetar. Los experimentos llevados a cabo utilizan el {\it MNIST} como origen y el
    {\it TDS} como destino. Se realizaron 3 particiones de cada uno:
...
El algoritmo de entrenamiento detallado en \ref{alg:experimento} se repite por cada uno de los experimentos planteados
anteriormente.

\begin{algorithm}
    \caption{Experimento}
    \label{alg:experimento}
    \begin{algorithmic}
        \STATE mejor\_loss $\gets$ 999.999
        \STATE mejor\_modelo $\gets$ NULL
        \FOR{iteracion en max\_iters\_optimizacion}
        \STATE params $\gets$ estimar\_params()
        \STATE mejor\_loss\_epoch $\gets$ 999.999
        \STATE counter $\gets$ 0
        \FOR{epoch en epochs}
        \STATE modelo $\gets$ entrenar\_epoch($MNIST_{train}$, $TDS_{train}$, params)
        \STATE loss $\gets$ validar(modelo, $MNIST_{val}$, $TDS_{val}$)
        \IF{loss < mejor\_loss\_epoch}
        \STATE mejor\_loss\_epoch $\gets$ loss
        \STATE mejor\_modelo\_epoch $\gets$ modelo
        \STATE counter $\gets$ 0
        \ELSE
        \STATE counter++
        \IF{counter > early\_stopping}
        \STATE break
        \ENDIF
        \ENDIF
        \ENDFOR
        \IF{mejor\_loss\_epoch < mejor\_loss}
        \STATE mejor\_loss $\gets$ mejor\_loss\_epoch
        \STATE mejor\_modelo $\gets$ mejor\_modelo\_epoch
        \ENDIF
        \ENDFOR
        \STATE evaluar(mejor\_modelo, $MNIST_{test}$, $TDS_{test}$)
    \end{algorithmic}
