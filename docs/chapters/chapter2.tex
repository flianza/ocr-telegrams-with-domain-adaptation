...
segunda es una generadora de características $\mathcal{G}$ que se entrena para confundir a $\mathcal{D}$ y
...
$\mathcal{L}_\mathcal{D}$. $\mathcal{L}_\mathcal{C}$ mide el error que posee la red en la clasificaci\'on de los datos
de origen y viene dada por el {\it cross-entropy} $\mathcal{L}_{CE}$ aplicado a la salida de la red $\mathcal{C}$. El
...
Por lo tanto, el $\mathcal{L}_\mathcal{G}$ viene dado por la ecuaci\'on \ref{eq:dann-loss-generadora}. Donde $\lambda$
...
que es an\'aloga a \ref{eq:dann-loss-discriminadora}. Finalmente, la ecuaci\'on \ref{eq:adda-loss-generadora} contiene
