...
enfoques anteriores. \citet{lecun1995learning} se menciona por primera vez el término de {\it red convolucional}.
...
\citet{mcculloch1943logical} estudiaron y propusieron un modelo del comportamiento de la
...
Años más tarde, \citet{rosenblatt1958perceptron} propone un modelo de neurona llamado perceptrón basado en el modelo de
\citeauthor{mcculloch1943logical}. El mayor logro de \citeauthor{rosenblatt1958perceptron} fue que pudo demostrar que,
flexibilizando algunas de las reglas de \citeauthor{mcculloch1943logical} (la inhibición absoluta, la contribución
igual de todas las entradas, etc.), las neuronas artificiales podían aprender de los datos. Y lo que es más importante,
ideó un algoritmo de aprendizaje supervisado para este modelo de neurona que permitía averiguar los pesos correctos
directamente a partir de los datos de entrenamiento. La sencillez y eficacia de este algoritmo de aprendizaje para
problemas linealmente separables son algunas de las razones clave por las que se hizo tan popular a finales de los años
cincuenta y principios de los sesenta. Sin embargo, esta popularidad hizo que Rosenblatt exagerara la capacidad de
aprendizaje de su perceptrón, dando lugar a expectativas poco realistas en la comunidad científica y/o también
...
backpropagation} desarrollada por \citet{rumelhart1986learning}. En realidad, esta técnica fue desarrollada
inicialmente por \citet{werbos1974beyond} y después redescubierta por varios grupos de investigadores \parencite{lecun1985learning, rumelhart1986learning}. Este algoritmo abrió las puertas al uso práctico de las redes
...
\begin{figure}[H]
    ...
    \end{tikzpicture}
    \caption[Red neuronal densa]{Red neuronal con una capa de entrada de cuatro características, una capa oculta de dos neuronas y una capa de salida con una neurona.}
    \label{fig:nn}
\end{figure}
...
\begin{itemize}
    ...
    \item Softmax: $f(x_i) = \sigma(x_i) = \frac{e^{x_{i}}}{\sum_{j=1}^K e^{x_{j}}}$ para $i=1,2,\dots,K$ donde $K$ son la
...
$g$ invertida a través de la función $f$ y multiplicar cada valor de $f$ por el valor de $g$ en esa posición. El resultado de la
...
características relevantes $I*K$ para la tarea en cuestión a través del entrenamiento. Esto permite que la red
encuentre características sin el sesgo que se introduciría al utilizar únicamente filtros predefinidos.
...
convolucional $\mathcal{G}$ que extrae información y genera características $\mathbf{z}$ a partir de la entrada
$\mathbf{x}$; y una subred densa $\mathcal{C}$ que se encarga de clasificar las características aprendidas $\mathbf{z}$
...
\begin{figure}[H]
    ...
    \caption[Red neuronal convolucional]{Arquitectura de una red convolucional simple. Las entradas $\mathbf{x}$ son procesadas por una red convolucional $\mathcal{G}$
        que generan características $\mathbf{z}$ para luego ser clasificadas por una red densa $\mathcal{C}$ en $\mathbf{\hat{y}}$.}
    \label{fig:cnn}
\end{figure}

Tal es el caso de la red LeNet, que fue propuesta por \citet{lecun1998gradient}. Esta red consta de dos capas de
...
Network), propuesta por \citet{he2016deep}. Estas redes están diseñadas para resolver el problema de la propagación del
...
\begin{figure}[H]
    ...
    \caption[Arquitectura de una LeNet5]{Arquitectura de una LeNet5. Imagen tomada de \citet{lecun1998gradient}.}
    \label{fig:lenet}
\end{figure}
...
complejidad y profundidad de la arquitectura: AlexNet con ocho capas y más de 60 millones de parámetros entrenables,
mientras que LeNet tiene cinco capas y menos de un millón de parámetros. Además, AlexNet utiliza filtros
convolucionales más grandes ($11 \times 11$ y $5 \times 5$) que permiten capturar características de imagen más grandes
y complejas, y técnicas de regularización más avanzadas, como el {\it dropout}, para prevenir el sobre ajuste. En
contraste, LeNet utiliza filtros más pequeños ($5 \times 5$ y $3 \times 3$) y técnicas de regularización más simples
como la regularización L2.

Desde el éxito de AlexNet, las CNN se han convertido en la mejor opción para aplicaciones de visión computacional y se
han creado muchos tipos diferentes de CNN, como las redes R-CNN utilizada para detección y segmentación de objetos \parencite{girshick2014rich}. En la actualidad los modelos CNN son bastante diferentes de LeNet original, pero todos se
desarrollan sobre la base generada por esta.
...
capas fijas en otra red que se entrena en una tarea diferente \parencite{yosinski2014transferable}, sin modificar los pesos de la red pre-entrenada. Esto permite que la red aprenda las
características específicas de la nueva tarea y con menos datos \parencite{howard2018universal}.
...
muestras generadas y las reales. Este proceso de entrenamiento adversario continúa hasta que se alcanza un punto de
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[x=0.75pt,y=0.75pt,yscale=-1,xscale=1]
        ...
        %Shape: Circle [id:dp8911007167211544] 
        \draw   (37.8,84.75) .. controls (37.8,76.33) and (44.63,69.5) .. (53.05,69.5) .. controls (61.47,69.5) and (68.3,76.33) .. (68.3,84.75) .. controls (68.3,93.17) and (61.47,100) .. (53.05,100) .. controls (44.63,100) and (37.8,93.17) .. (37.8,84.75) -- cycle ;
        %Shape: Trapezoid [id:dp17673865425968227] 
        \draw   (130,122) -- (90,110) -- (90,60) -- (130,48) -- cycle ;
        %Straight Lines [id:da17016942407473712] 
        \draw    (69.5,85.15) -- (86,85.15) ;
        \draw [shift={(89,85.15)}, rotate = 180] [fill={rgb, 255:red, 0; green, 0; blue, 0 }  ][line width=0.08]  [draw opacity=0] (8.93,-4.29) -- (0,0) -- (8.93,4.29) -- cycle    ;
        %Shape: Circle [id:dp0351848498318883] 
        \draw  [dash pattern={on 4.5pt off 4.5pt}] (155.38,86.08) .. controls (155.38,77.66) and (162.21,70.83) .. (170.63,70.83) .. controls (179.06,70.83) and (185.88,77.66) .. (185.88,86.08) .. controls (185.88,94.51) and (179.06,101.33) .. (170.63,101.33) .. controls (162.21,101.33) and (155.38,94.51) .. (155.38,86.08) -- cycle ;
        %Straight Lines [id:da08691567239156806] 
        \draw    (130.33,84.13) -- (151.98,84.13) ;
        \draw [shift={(154.98,84.13)}, rotate = 180] [fill={rgb, 255:red, 0; green, 0; blue, 0 }  ][line width=0.08]  [draw opacity=0] (8.93,-4.29) -- (0,0) -- (8.93,4.29) -- cycle    ;
        %Straight Lines [id:da48589972370276335] 
        \draw    (185.9,150.95) -- (216.51,139.97) ;
        \draw [shift={(219.33,138.95)}, rotate = 160.26] [fill={rgb, 255:red, 0; green, 0; blue, 0 }  ][line width=0.08]  [draw opacity=0] (8.93,-4.29) -- (0,0) -- (8.93,4.29) -- cycle    ;
        %Straight Lines [id:da7455647779284351] 
        \draw    (185.88,86.08) -- (216.56,98.67) ;
        \draw [shift={(219.33,99.81)}, rotate = 202.31] [fill={rgb, 255:red, 0; green, 0; blue, 0 }  ][line width=0.08]  [draw opacity=0] (8.93,-4.29) -- (0,0) -- (8.93,4.29) -- cycle    ;
        %Shape: Circle [id:dp6519338947936726] 
        \draw   (155.4,150.95) .. controls (155.4,142.53) and (162.23,135.7) .. (170.65,135.7) .. controls (179.07,135.7) and (185.9,142.53) .. (185.9,150.95) .. controls (185.9,159.37) and (179.07,166.2) .. (170.65,166.2) .. controls (162.23,166.2) and (155.4,159.37) .. (155.4,150.95) -- cycle ;

        %Shape: Trapezoid [id:dp7187159288939746] 
        \draw   (220.4,82) -- (260.4,94) -- (260.4,144) -- (220.4,156) -- cycle ;

        %Shape: Ellipse [id:dp7149271359322635] 
        \draw   (294.67,119) .. controls (294.67,107.95) and (310.34,99) .. (329.67,99) .. controls (349,99) and (364.67,107.95) .. (364.67,119) .. controls (364.67,130.05) and (349,139) .. (329.67,139) .. controls (310.34,139) and (294.67,130.05) .. (294.67,119) -- cycle ;
        %Straight Lines [id:da8042763294948492] 
        \draw    (260.67,119.33) -- (291.67,119.33) ;
        \draw [shift={(294.67,119.33)}, rotate = 180] [fill={rgb, 255:red, 0; green, 0; blue, 0 }  ][line width=0.08]  [draw opacity=0] (8.93,-4.29) -- (0,0) -- (8.93,4.29) -- cycle    ;

        % Text Node
        \draw (170,86) node   [align=left] {\begin{minipage}[lt]{21.08pt}\setlength\topsep{0pt}
                \begin{center}
                    $\hat{\mathbf{x}}$
                \end{center}
            ...
            \end{minipage}};
        % Text Node
        \draw (52,85) node   [align=left] {\begin{minipage}[lt]{21.08pt}\setlength\topsep{0pt}
                \begin{center}
                    $\mathbf{z}$
                \end{center}
        ...
        \draw (110,85) node   [align=left] {\begin{minipage}[lt]{22.78pt}\setlength\topsep{0pt}
            ...
            \end{minipage}};
        % Text Node
        \draw (170,151) node   [align=left] {\begin{minipage}[lt]{21.08pt}\setlength\topsep{0pt}
                \begin{center}
                    $\mathbf{x}$
                \end{center}
            ...
            \end{minipage}};
        % Text Node
        \draw (240.4,119) node   [align=left] {\begin{minipage}[lt]{22.78pt}\setlength\topsep{0pt}
                \begin{center}
                    $\mathcal{D}$
                \end{center}
            ...
            \end{minipage}};
        % Text Node
        \draw (300,110.33) node [anchor=north west][inner sep=0.75pt]   [align=left] {{\footnotesize real/falso}};

    \end{tikzpicture}

    \caption[Esquema de una GAN]{Esquema de una GAN. $\mathbf{z}$ es el espacio latente con el cual la red generadora $\mathcal{G}$ produce datos sintéticos $\hat{\mathbf{x}}$. Luego, la red discriminadora $\mathcal{D}$ intenta distinguir entre los datos sintéticos $\hat{\mathbf{x}}$ y los datos reales $\mathbf{x}$. Con el tiempo, $\mathcal{G}$ irá generando datos similares a los reales y $\mathcal{D}$ no podrá distinguir los unos de los otros.}
...
$\mathcal{L}_{adv}$ dará lugar a representaciones transferibles.
...
\ref{eq:dann-loss-clasificadora}.
...
\noindent
La función de costo de la discriminación adversaria $\mathcal{L}_{adv}$ detallada a continuación es análoga a la
ecuación \ref{eq:dann-loss-discriminadora} de {\it DANN}.
...
\noindent
Finalmente, la ecuación \ref{eq:adda-objetivo} contiene el objetivo a optimizar de un modelo {\it ADDA}, que asigna
gradientes pequeños a registros de destino que sean similares a los de origen y gradientes más grandes para los otros
...
respectivamente del análisis espectral $\mathbf{z} = \mathsf{U} \Sigma \mathsf{V}^\intercal $. En la presente tesis se utilizará el valor por defecto propuesto por \citeauthor{chen2019transferability} de $k=1$.
...
dominio de origen, mientras que haga predicciones distintas a $\mathcal{C}$ en los datos del dominio destino. Mediante
...
\begin{figure}[H]
    ...
    \caption[Esquema de {\it MDD}]{Esquema de {\it MDD}. Los círculos con líneas rayadas son salidas de los modelos mientras que los círculos de líneas sólidas son datos conocidos.
        ...
        minimizar el error de origen como también la discrepancia.}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{chapter2/afn-visda-source-only.png}

    \caption[Representación de características aprendidas de una CNN]{Representación t-SNE de dos dimensiones de las características aprendidas para el origen y destino utilizando un modelo entrenado en muestras del origen para el conjunto de datos VisDA \parencite{visda2017}. Imagen tomada de \citet{xu2019larger}.} \label{fig:afn-source-only}
\end{figure}
